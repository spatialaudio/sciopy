...
def conf_n_el_48_adjacent(
    ...
    Switch Type 	Reed Relays
    """
    serial.write(bytearray([0xB0, 0x01, 0x01, 0xB0])) #reset setup
    serial.write(bytearray([0xB0, 0x03, 0x02, 0x00, 0x01, 0xB0])) #burst count 2 Byte integer
    #excitation amplitude (double Precision)
    serial.write(bytearray([0xB0, 0x09, 0x05, 0x3F, 0x50, 0x62, 0x4D, 0xD2, 0xF1, 0xA9, 0xFC, 0xB0])) 
    serial.write(bytearray([0xB0, 0x02, 0x0D, 0x02, 0xB0])) #ADC  range +- 5 v 
    serial.write(bytearray([0xB0, 0x03, 0x09, 0x01, 0x00, 0xB0]))  #global gain #Data gain = 1 byte
    serial.write(bytearray([0xB0, 0x03, 0x08, 0x01, 0x01, 0xB0])) #adjacent
    serial.write(bytearray([0xB0, 0x02, 0x0C, 0x01, 0xB0])) #Excitation switch type = default
    #Frame rate 4 Byte integer 0x41, 0x20, 0x00, 0x00 = 10 
    serial.write(bytearray([0xB0, 0x05, 0x03, 0x41, 0x20, 0x00, 0x00, 0xB0])) 
    #excitation frequencies #fmin=10khz  #fmax=10khz  #fcount=01 #ftype=0 (linear frequency                                 
    serial.write(bytearray([0xB0, 0x0C, 0x04, 0x46, 0x1C, 0x40, 0x00, 0x46, 0x1C, 0x40, 0x00, 0x00, 0x01, 0x00, 0xB0])) 
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x01, 0x02, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x02, 0x03, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x03, 0x04, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x04, 0x05, 0xB0]))
    ...
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x30, 0x01, 0xB0]))
...
def conf_n_el_48_opposite(
    ...
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x01, 0x19, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x02, 0x1A, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x03, 0x1B, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x04, 0x1C, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x05, 0x1D, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x06, 0x1E, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x07, 0x1F, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x08, 0x20, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x09, 0x21, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x0A, 0x22, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x0B, 0x23, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x0C, 0x24, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x0D, 0x25, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x0E, 0x26, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x0F, 0x27, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x10, 0x28, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x11, 0x29, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x12, 0x2A, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x13, 0x2B, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x14, 0x2C, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x15, 0x2D, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x16, 0x2E, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x17, 0x2F, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x18, 0x30, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x19, 0x01, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x1A, 0x02, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x1B, 0x03, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x1C, 0x04, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x1D, 0x05, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x1E, 0x06, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x1F, 0x07, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x20, 0x08, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x21, 0x09, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x22, 0x0A, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x23, 0x0B, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x24, 0x0C, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x25, 0x0D, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x26, 0x0E, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x27, 0x0F, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x28, 0x10, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x29, 0x11, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x2A, 0x12, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x2B, 0x13, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x2C, 0x14, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x2D, 0x15, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x2E, 0x16, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x2F, 0x17, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x30, 0x18, 0xB0]))
...
def conf_n_el_64_adjacent(
    ...
    serial.write(bytearray([0xB0, 0x01, 0x01, 0xB0])) #reset setup
    serial.write(bytearray([0xB0, 0x03, 0x02, 0x00, 0x01, 0xB0])) #burst count 2 Byte integer
    serial.write(bytearray([0xB0, 0x09, 0x05, 0x3F, 0x50, 0x62, 0x4D, 0xD2, 0xF1, 0xA9, 0xFC, 0xB0])) #excitation amplitude (double Precision)
    #0x3F, 0x50, 0x62, 0x4D, 0xD2, 0xF1, 0xA9, 0xFC what is that?
    serial.write(bytearray([0xB0, 0x02, 0x0D, 0x02, 0xB0])) #ADC  range +- 5 v 
    serial.write(bytearray([0xB0, 0x03, 0x09, 0x01, 0x00, 0xB0]))  #global gain #Data gain = 1 byte
    serial.write(bytearray([0xB0, 0x03, 0x08, 0x01, 0x01, 0xB0])) #adjacent
    serial.write(bytearray([0xB0, 0x02, 0x0C, 0x01, 0xB0])) #Excitation switch type = default
    serial.write(bytearray([0xB0, 0x05, 0x03, 0x41, 0x20, 0x00, 0x00, 0xB0])) #Frame rate 4 Byte integer 0x41, 0x20, 0x00, 0x00 = 10 
    serial.write(bytearray([0xB0, 0x0C, 0x04, 0x46, 0x1C, 0x40, 0x00, 0x46, 0x1C, 0x40, 0x00, 0x00, 0x01, 0x00, 0xB0])) #excitation frequencies #fmin=10khz 
                                                                                                                                                #fmax=10khz 
                                                                                                                                                #fcount=01
                                                                                                                                                #ftype=0 (linear frequency)
...
def conf_n_el_64_opposite(
    ...
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x01, 0x21, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x02, 0x22, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x03, 0x23, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x04, 0x24, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x05, 0x25, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x06, 0x26, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x07, 0x27, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x08, 0x28, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x09, 0x29, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x0A, 0x2A, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x0B, 0x2B, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x0C, 0x2C, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x0D, 0x2D, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x0E, 0x2E, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x0F, 0x2F, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x10, 0x30, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x11, 0x31, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x12, 0x32, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x13, 0x33, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x14, 0x34, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x15, 0x35, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x16, 0x36, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x17, 0x37, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x18, 0x38, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x19, 0x39, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x1A, 0x3A, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x1B, 0x3B, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x1C, 0x3C, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x1D, 0x3D, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x1E, 0x3E, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x1F, 0x3F, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x20, 0x40, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x21, 0x01, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x22, 0x02, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x23, 0x03, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x24, 0x04, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x25, 0x05, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x26, 0x06, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x27, 0x07, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x28, 0x08, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x29, 0x09, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x2A, 0x0A, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x2B, 0x0B, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x2C, 0x0C, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x2D, 0x0D, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x2E, 0x0E, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x2F, 0x0F, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x30, 0x10, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x31, 0x11, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x32, 0x12, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x33, 0x13, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x34, 0x14, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x35, 0x15, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x36, 0x16, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x37, 0x17, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x38, 0x18, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x39, 0x19, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x3A, 0x1A, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x3B, 0x1B, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x3C, 0x1C, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x3D, 0x1D, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x3E, 0x1E, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x3F, 0x1F, 0xB0]))
    serial.write(bytearray([0xB0, 0x03, 0x06, 0x40, 0x20, 0xB0]))
